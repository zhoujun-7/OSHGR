...
class MaskAutoEncoder(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        ## x: (B, 49, 1024)
        ## mask: (2, 196) 0 is keep, 1 is remove
        ## ids_restore: (2, 196)
    ...
    def forward_decoder(self, x, ids_restore):
        # embed tokens
        x = self.decoder_embed(x)  # 512

        # append mask tokens to sequence
        mask_tokens = self.mask_token.repeat(x.shape[0], ids_restore.shape[1] + 1 - x.shape[1], 1)
        x_ = torch.cat([x[:, 1:, :], mask_tokens], dim=1)  # no cls token  # (B, 196, 512)

        x_ = torch.gather(
            x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2])
        )  # unshuffle  # (B, 196, 512)
        x = torch.cat([x[:, :1, :], x_], dim=1)  # append cls token  # (B, 197, 512)
    ...
    def forward(self, imgs, noise, mask_ratio=0.75):
        latent, mask, ids_restore = self.forward_encoder(noise, mask_ratio)
