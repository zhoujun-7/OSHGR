...
class MyNet(nn.Module):
    ...
    def forward(self, x={}, gt={}, phase="pretrain", save=False):
        pd = {}
        loss = {}
        if phase == "pretrain":
            if self.MAE_USE:
            ...
            if not self.ONLY_MAE:
                ...
                if self.GCN_USE:
                    ...
                    # pd_emb = torch.cat([pd_emb, pd_independ_kpt.reshape(-1,21*3)], dim=-1)
                    # pd["emb"] = pd_emb
                    pd["kpt"] = pd_kpt
                    pd["ang"] = pd_ang
            ...
            if self.GCN_USE:
                ...
                pd_emb = torch.cat([pd_emb, pd["ang2kpt"][:, MANO_KPT_TO_ANGLE_ID2].reshape(-1,15*3)], dim=-1)
                # pd_emb = pd["ang2kpt"][:, MANO_KPT_TO_ANGLE_ID2].reshape(-1,15*3)
                pd["emb"] = pd_emb
            else:
                pd["emb"] = feat_gcn
