...
class ViewPointRPY:
    ...
    def run(self):
        ...
        testset = ViewPointRPY(TYPE=VIEW_TYPE, SESSION=9, IS_AUGMENT=False)
        ...
        np.save(
            f"log/Train/Ours/best/{VIEW_TYPE}_prediction_{self.cfg.DATASET.INCR_SHOT}",
            all_pred.cpu().numpy(),
        )
        np.save(
            f"log/Train/Ours/best/{VIEW_TYPE}_view_point_{self.cfg.DATASET.INCR_SHOT}",
