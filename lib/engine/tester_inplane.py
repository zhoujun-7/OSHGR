...
class InplaneTester:
    ...
    def run(self):
        ...
        angle_ls = []
        acc_ls = []
        for r in range(0, 361, ROT):
            testset = ConstrainedFSCIHGR(
                PHASE="test",
                SESSION=9,
                ...
                ERASE_MU=(-3, 3),
                ERASE_SIGMA=(30, 80),
                SMOOTH_PROBABILITY=0,
                SMOOTH_KERNEL=(2, 5),
                IS_AUGMENT=True,
            )
            testloader = torch.utils.data.DataLoader(
                dataset=testset, batch_size=self.cfg.TRAIN.PER_GPU_BATCH, shuffle=False, num_workers=8, pin_memory=True
            )
            ...
            with torch.no_grad():
                for image, noise_image, label, pose, angle in tqdm.tqdm(
                    testloader, desc=f"test inplane {r} class {len(testset)}"
                ):
                    x_dt = {"image": image.to(self.RANK)}
                    gt_dt = {"class": label.to(self.RANK)}
                    pd_dt, loss_dt = self.model(x_dt, gt_dt, phase="test")
            ...
            all_pred = torch.argmax(all_logit, dim=1)
            acc = (all_pred == all_label).to(torch.float32).mean().item()
        ...
        np.save(
            f"log/Train/Ours/best/inplane_acc_{self.cfg.DATASET.INCR_SHOT}",
            acc_ls,
        )
        np.save(
            f"log/Train/Ours/best/inplane_angle_{self.cfg.DATASET.INCR_SHOT}",
            angle_ls,
        )
