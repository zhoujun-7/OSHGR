...
class Finetune_Trainer(BaseTrainer):
    ...
    def incr_shape(self):
        ...
        for i, shape in enumerate(shape_ls):
            ...
            for epoch in range(1000):
                for epi, (image, noise_image, label, pose, angle) in enumerate(incr_shape_dataloader):
    ...
    def incr_class(self):
        extra_weight = torch.rand_like(torch.tile(self.model.fc.weight.data[[0]], [45, 1]))
        extra_bias = torch.rand_like(torch.tile(self.model.fc.bias.data[[0]], [45]))
        extra_weight = nn.init.kaiming_uniform_(extra_weight, a=math.sqrt(5))
        extra_weight = nn.init.kaiming_uniform_(extra_weight, a=math.sqrt(5))
        ...
        # print(self.model.fc.weight.data.shape)
        ...
        for sess in range(1, 4):
            ...
            print("inremental class :", np.unique(incr_class_dataset.label_ls))
            ...
            for epoch in tqdm.trange(4000):
                for epi, (image, noise_image, label, pose, angle) in enumerate(incr_class_dataloader):
                    with torch.autocast(device_type="cuda", dtype=self.cfg.TRAIN.DTYPE):
                        ...
                        y = self.ddp_model(image.to(self.RANK))
                        y = torch.softmax(y[:, :(23+15*sess)], dim=-1)
                        loss_final = torch.nn.functional.cross_entropy(y, label)
    ...
    def test_class(self, session):
        ...
        print(np.unique(test_class_dataset.label_ls))
        all_logit = []
        all_label = []
        with torch.no_grad():
            for image, noise_image, label, pose, angle in tqdm.tqdm(
                ...
                # print(y.shape)
                # exit()
                y = torch.softmax(y[:, :(23+15*session)], dim=-1)
...
class ConfMatData:
    ...
    def write(self):
        ...
        save_path = os.path.join(self.path, 'Finetune_15.pth')
        print('*'*100)
        print(save_path)
