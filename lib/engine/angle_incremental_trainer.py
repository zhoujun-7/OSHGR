    ...
    def test_class(self, session):
        ...
        with torch.no_grad():
            ...
            for image, noise_image, label, pose, angle, timestamp in tqdm.tqdm(
                ...
                # timestamp_ls.extend(timestamp[m])

                # region [240116]
                # _label = label.detach().cpu().numpy()
                # path = "out/joint-weight/label/" + f"{i}.npz"
                # np.savez(path, label=_label)
                # i += 1
                # endregion [240116]
        ...
        # print((sess_pred == sess_label).shape)

        # dt = {'time': timestamp_ls, 'acc': (sess_pred == sess_label)}
        # import pickle
        # with open("./time_n_acc_25.pkl", 'wb') as f:
        #     pickle.dump(dt, f)
        return class_acc_dt


    def test_class_(self, session):
        if self.cfg.TRAIN.TASK == "CFSCI":
            FSCIHGR = ConstrainedFSCIHGR
        elif self.cfg.TRAIN.TASK == "UFSCI":
            FSCIHGR = UnconstrainedFSCIHGR
        ...
        with torch.no_grad():
            ...
            for image, noise_image, label, pose, angle, timestamp in tqdm.tqdm(
                ...
                m = torch.logical_and(label < 53, label >= 0)
                for j in range(len(timestamp)):
                    if m[j]:
                        timestamp_ls.append(timestamp[j])
                # timestamp_ls.extend(timestamp[m])
        ...
        for sess in range(session + 1):
            ...
            m = torch.logical_and(all_label < 53, all_label >= 0)
            sess_label = all_label[m]
            # sess_pred = torch.argmax(all_logit[m][:, :num_test_class], dim=1)
            sess_pred = torch.argmax(all_logit[m][:, 0:53], dim=1)
            # print(sess_label)
            # print(sess_pred)
        ...
        # dt = {'time': timestamp_ls, 'acc': (sess_pred == sess_label).cpu().numpy()}
        # import pickle
        # with open("./time_n_acc_30.pkl", 'wb') as f:
        #     pickle.dump(dt, f)
        return class_acc_dt
