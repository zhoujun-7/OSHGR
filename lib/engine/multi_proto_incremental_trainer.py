...
class MultiProtoIncrementalTrainer:
    ...
    def incr_shape(self):
        ...
        for shape in shape_ls:
            ...
            # self.model.classifier.restore_proto(is_kaiming=False)
    ...
    def incr_class(self):
        ...
        for sess in range(1, self.cfg.DATASET.NUM_SESSION + 1):
            ...
            augment_proto_pt = self.do_augment_proto(
                incr_class_dataloader, self.cfg.AUGMENT_PROTO.N_INCR_DATA, self.cfg.AUGMENT_PROTO.N_PROTO
            )

            augment_proto_pt = augment_proto_pt.to(self.RANK)
            self.model.classifier.proto.weight.data = torch.cat(
                [self.model.classifier.proto.weight.data, augment_proto_pt]
            )
    ...
    def test_class(self, session):
        ...
        class_acc_dt = {}
        for sess in range(self.cfg.DATASET.NUM_SESSION + 1):
            num_test_class = self.cfg.DATASET.NUM_BASE_CLASS + self.cfg.DATASET.INCR_WAY * sess
            m = all_label < num_test_class

            print(num_test_class * self.cfg.AUGMENT_PROTO.N_PROTO)

            sess_label = all_label[m]
            sess_pred = torch.argmax(all_logit[m][:, : num_test_class * self.cfg.AUGMENT_PROTO.N_PROTO], dim=1)

            sess_pred = sess_pred // self.cfg.AUGMENT_PROTO.N_PROTO
            ...
            print("*" * 100)
            print(sess)
            print(sess_label[::100])
            print(sess_pred[::100])
            mask = sess_label != sess_pred
            print(sess_label[mask])
            print(sess_pred[mask])

        return class_acc_dt

    def do_augment_proto(self, dataloader, n_augment_data, n_augment_proto):
        with torch.no_grad():
            embedding_list = []
            label_list = []
            device = self.model.classifier.proto.weight.data.device

            desc = str(dataloader.dataset).split(" object")[0].split(".")[-1] + f": {len(dataloader.dataset)}"
            for i in range(n_augment_data):
                if i == 0:
                    dataloader.dataset.IS_AUGMENT = False
                else:
                    dataloader.dataset.IS_AUGMENT = True
                for data in tqdm.tqdm(dataloader, desc=desc):
                    x = {"image": data[0].to(device)}
                    gt = {"class": data[2].to(device)}
                    pd, loss = self.model(x, gt, phase="test")

                    embedding_list.append(pd["emb"])
                    label_list.append(gt["class"])
            embedding_list = torch.cat(embedding_list, dim=0)
            label_list = torch.cat(label_list, dim=0)

            print(embedding_list.shape)
            print(label_list.unique())

            augment_proto_ls = []
            for la in label_list.unique():
                mask = label_list == la
                _, augment_proto = kmeans(X=embedding_list[mask], num_clusters=n_augment_proto, device=device)
                augment_proto_ls.append(augment_proto)

            augment_proto_ls = torch.cat(augment_proto_ls)
            return augment_proto_ls

    def augment_base_proto(self):
        base_dataset = UnconstrainedPretrain(
        ...
        augment_proto_pt = self.do_augment_proto(
            self.base_loader, self.cfg.AUGMENT_PROTO.N_BASE_DATA, self.cfg.AUGMENT_PROTO.N_PROTO
        )

        self.model.classifier.proto.weight.data = augment_proto_pt.to(self.RANK)
