...
class IncrementalTrainer:
    ...
    def test_shape(self, shape_ls):
        ...
        for i, shape in enumerate(shape_ls):
            ...
            test_shape_dataset.label_ls += self.cfg.DATASET.NUM_BASE_CLASS * (i + 1)
        ...
        with torch.no_grad():
            for image, noise_image, label, pose, angle in tqdm.tqdm(
                ...
                test_embed_ls.append(pd_dt['emb'].cpu())

        all_logit = torch.cat(all_logit)
        all_label = torch.cat(all_label)
        test_embed_ls = torch.cat(test_embed_ls)
        
        # print(len(shape_ls))
        if len(shape_ls) == 4:
            proto_ls = self.model.classifier.proto.weight.data.clone().detach().cpu()
            data_ls = [test_embed_ls, all_label.cpu(), proto_ls]
            torch.save(data_ls, os.path.join(self.cfg.LOG.DIR, 'tsne_shape_data_ours.pth'))
            print(proto_ls[5::23, ::200])
    ...
    def test_class(self, session):
        ...
        train_dataloader = DataLoader(
            ...
            shuffle=False,)
        with torch.no_grad():
            for image, noise_image, label, pose, angle in tqdm.tqdm(train_dataloader):
                x_dt = {"image": image.to(self.RANK)}
                gt_dt = {"class": label.to(self.RANK)}
                pd_dt, loss_dt = self.model(x_dt, gt_dt, phase="test")
                train_emb_ls.append(pd_dt['emb'].cpu())
                train_label_ls.append(label.cpu())
        train_emb_ls = torch.cat(train_emb_ls)
        ...
        with torch.no_grad():
            for image, noise_image, label, pose, angle in tqdm.tqdm(
                ...
                all_logit.append(pd_dt["logit"])
                all_label.append(gt_dt["class"])
                test_emb_ls.append(pd_dt['emb'].cpu())
        test_emb_ls = torch.cat(test_emb_ls)
        test_label_ls = torch.cat(all_label).cpu()

        proto_ls = self.model.classifier.proto.weight.data.clone().detach().cpu()

        data_ls = [train_emb_ls, train_label_ls, test_emb_ls, test_label_ls, proto_ls]

        torch.save(data_ls, os.path.join(self.cfg.LOG.DIR, 'tsne_class_data_ours.pth'))
