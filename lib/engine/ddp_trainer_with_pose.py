...
from ..dataset.oshgr_pose import UnconstrainedPretrain, UnconstrainedFSSIHGR, UnconstrainedFSCIHGR
...
class DDP_Trainer(BaseTrainer):
    ...
    def test_shape(self, shape):
        ...
        all_label = []
        with torch.no_grad():
            for image, noise_image, label, pose, angle in tqdm.tqdm(test_shape_dataloader, desc=f"test shape {shape}"):
    ...
    def log_image(self, x_dt, pd_dt, gt_dt):
        ...
        if self.cfg.MODEL.GCN.USE:
            ...
            log_img.append(draw_image_pose(local_pose_2_image_pose(pd_dt["ang2kpt"][0]), background="white"))
        if self.cfg.MODEL.MAE.USE:
