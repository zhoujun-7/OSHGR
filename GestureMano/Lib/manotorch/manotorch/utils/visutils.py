...
def display_hand_pyrender(
    ...
    if show_axis:
        if bul_axes is not None:
            ...
            # b_axis = b_axis * 0 + np.array([1,0,0])
            # u_axis = u_axis * 0 + np.array([0,1,0])
            # l_axis = l_axis * 0 + np.array([0,0,1])
            for i in range(15):
        ...
        bb, uu, ll = eye
        tt = np.eye(4, 4)
        draw_axis(bb, tt, scene, color=np.array([255, 42, 34, 255]))
        draw_axis(uu, tt, scene, color=np.array([190, 255, 0, 255]))
        draw_axis(ll, tt, scene, color=np.array([23, 217, 255, 255]))
